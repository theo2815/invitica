    ...
    generation = await provider.generate(
      {
        ...
        messages: [{ content: currentDraftMessage(draft.document), role: "user" }, ...messages],
        outputSchema: buildProposalSchema(draft.document, draft.manifest),
        systemPrompt: documentSystemPrompt(draft.document, draft.manifest),
      },
      request.signal,
    );
  } catch (error) {
