  ...
  if (type === "event-details") {
    ...
    return {
      ...
      events: events.map((event, index) => {
        ...
        if (!isRecord(event) || source?.latitude === undefined || source.longitude === undefined) {
          return event;
        }
        return { ...event, latitude: source.latitude, longitude: source.longitude };
      }),
    };
  }
...
export function resolveDocumentProposal(
  ...
  for (const [type, entry] of Object.entries(output)) {
  ...
  const parsed = sectionDocumentDetailsSchema.safeParse(carried);
